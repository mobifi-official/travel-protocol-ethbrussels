...
export default function HotelBookerComponent() {
    ...
    const [isWantsToBookHotel, setIsWantToBookHotel] = useState<boolean>(false);
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-[12px] bg-white rounded-[12px]">
            ...
            <div className="flex flex-col items-center justify-start gap-[12px] bg-white/30 p-12 shadow-xl ring-1 ring-gray-900/5 rounded-lg backdrop-blur-lg max-w-xl mx-auto w-full">
                {(!isWantsToBookHotel) &&
                    <Button className="bg-green-600 text-white font-bold p-4 h-[48px] rounded-full"
                        onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setIsWantToBookHotel(true);
                        }}
                    >
                        Book A Hotel
                    </Button>}
                {(isWantsToBookHotel) && <div className="flex flex-col items-center justify-center gap-[12px]">
                    <div className="flex flex-col p-4 items-start justify-start  bg-white text-black border-[1px] border-black rounded-[8px]">
                        <p>Hotel Brussels 2024#ETH</p>
                        <div className="h-[20px]"></div>
                        <p>1 Room x 2 Nights</p>
                        <div className="h-[12px]"></div>
                        <p>EUR 129.43</p>
                    </div>
                    {(!bookingProofData) && <Button
                        className="bg-green-600 text-white font-bold p-4 h-[48px] rounded-full"
                        onClick={async (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            try {
                                ...
                                setModalContent(
                                    <div className="flex flex-col items-center justify-center gap-[12px]">
                                        ...
                                        <p className="text-green-500 text-center">Booking Successful!</p>
                                    </div>
                                );
                    ...
                    {(bookingProofData) && <p>{`Your Booking ID: BK_ETHGLOBAL2024`}</p>}
                </div>}
            </div>
